...
Веб-сервис для визуализации предсказаний ML-модели и индекса роста бизнеса по гексагональной сетке H3 над картой Москвы. Реализует сценарий: выбор категории бизнеса → тепловая карта → клик по гексагону → карточка с полной аналитикой и SHAP-объяснением.
...
3. Клик по гексагону — карточка показывает:
   ...
   - топ-5 SHAP-факторов с горизонтальными барами (кроме слоя growth_index)
...
| GET | `/api/topk` | `category`, `n=10` | Топ-N локаций по `score` |
